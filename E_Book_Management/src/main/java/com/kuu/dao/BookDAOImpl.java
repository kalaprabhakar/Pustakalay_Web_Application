...
public class BookDAOImpl implements IBooksDAO {
	...
	private static final String DELETE_OLD_BOOK = "DELETE FROM BOOK_DTLS WHERE EMAIL =? AND BOOKID = ?";
	private static final String SEARCH_BOOK = " SELECT * FROM BOOK_DTLS WHERE BOOKNAME LIKE ? OR AUTHOR LIKE ? OR BOOKCATEGORY LIKE ? AND STATUS = ?";
	...
	public boolean oldBookDelete(String email, int bookId) {
		...
		try {
			...
			Connection conn = ConnectionFactory.getConnection();
			PreparedStatement ps  = conn.prepareStatement(DELETE_OLD_BOOK);
